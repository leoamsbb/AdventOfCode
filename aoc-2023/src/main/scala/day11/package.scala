...
package object day11 {
  ...
  class Monkey(val num: Int,
    ...
    private def applyOperation(currentWorryLevel: Long) = {
      def toNum: String => Long = _.toLong
//      def toNum: String => BigInt = BigInt(_)
      ...
      operation match {
        ...
        case "- old" => 0
        case "/ old" => 1
        case s"* $num" => lcm(currentWorryLevel, toNum(num))
        case s"+ $num" => currentWorryLevel + toNum(num)
        case s"- $num" => currentWorryLevel - toNum(num)
        case s"/ $num" => lcm(currentWorryLevel / toNum(num), toNum(num))
      }
    }
...
//      def toNum: String => Long = Long(_)
      ...
      val Seq(num: Int, items: List[Long], operation: String, test: Int, throwToIfTrue: Int, throwToIfFalse: Int) = input
        ...
        .map {
          ...
          case s"Starting items: $listOfItems" => listOfItems.split(",").map(_.trim).map(toNum).toList
